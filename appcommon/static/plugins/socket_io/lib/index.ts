...
import {
  ...
  Room,
  SocketId,
} from "socket_io-adapter";
import * as parser from "socket_io-parser";
import type { Encoder } from "socket_io-parser";
...
const debug = debugModule("socket_io:server");
...
> {
  ...
  /**
   * Attaches socket_io to a server or port.
   *
  ...
  public attach(
    ...
    if ("function" == typeof srv) {
      const msg =
        "You are trying to attach socket_io to an express " +
    ...
    // set engine.io path to `/socket_io`
  ...
  /**
   * Binds socket_io to an engine.io instance.
   *
